...
if not getgenv().econix_loaded then
    getgenv().econix_loaded = true
else
...
local isNew = false
pcall(function()
    if not isfolder("econix_linoria") then makefolder("econix_linoria"); isNew = true; end
    for _, v in pairs({"econix_linoria/ink_game", "econix_linoria/themes"}) do
        if not isfolder(v) then makefolder(v); isNew = true; end
    end
    for _, v in pairs({"econix_linoria/ink_game/settings", "econix_linoria/ink_game/themes"}) do
        if not isfolder(v) then makefolder(v); isNew = true; end
    end

    if isNew then
        writefile("econix_linoria/themes/default.txt", "Jester")
        local suc = pcall(function()
            writefile("econix_linoria/ink_game/settings/default.json", game:HttpGet("https://raw.githubusercontent.com/Erchobg/VoidwareProfiles/refs/heads/main/InkGame/ink_game/settings/default.json", true))
        end)
        if suc then
            writefile("econix_linoria/ink_game/settings/autoload.txt", "default")
        end
    end
end)
...
if targetlib ~= "Wind" then
    ...
    ThemeManager:SetFolder("econix_linoria")
    SaveManager:SetFolder("econix_linoria/ink_game")
    ...
    InformationGroup:AddButton("Reset Settings", function()
        if approved then
            pcall(function()
                writefile("econix_linoria/ink_game/settings/default.json", "[]")
            end)
            pcall(function()
                Library:Unload()
            end)
